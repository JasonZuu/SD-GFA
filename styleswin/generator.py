...
from styleswin.basic_layers import (EqualLinear, PixelNorm,
                                 SinusoidalPositionalEmbedding, Upsample)
from utils.face_tools import reverse_imagenet_norm
...
class StyleSwinGenerator(nn.Module):
    ...
    def original_forward(
        ...
        if return_latents:
            return image, style_latents
        else:
            return image
    
    def get_style_latents(self, noise):
        styles = self.style(noise)
        return styles
    ...
    def forward(self,
        ...
        inject_index=None):
        """
        The forward function for swin-based face anonymization model
        change the input from random noise to style features
        also delete the truncation
        """
        inject_index = self.n_latent
        ...
        if reverse_norm:
            image = reverse_imagenet_norm(image)
...
def test_styleswin_genereator():
    # styleswin_pth = 'data/checkpoints/CelebAHQ_256.pt'
    generator = StyleSwinGenerator(size=256, style_dim=512, n_mlp=8) # default settings
    # state_dict = torch.load(styleswin_pth)
    # generator.load_state_dict(state_dict['g'])
    generator.to('cuda')
    noise = torch.randn(2, 512).to('cuda')
    img = generator(noise, reverse_imagenet_norm=True)
    for i in range(img.shape[0]):
        torchvision.utils.save_image(img[i],
                        f'styleswin_test{i}.png',
                        nrow=1,
                        padding=0,
                        range=(0, 1),)
